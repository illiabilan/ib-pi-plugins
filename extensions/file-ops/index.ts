...
export default function (pi: ExtensionAPI) {
	pi.registerTool({
		...
		promptGuidelines: [
			...
			"Do not ask for permission in prose before calling — the tool asks the user itself (a confirm dialog in interactive sessions, so the whole thing is one decision). If it instead returns a plan plus a token, that plan is a message TO THE USER: relay it (with any '!! HIGH RISK' block and any 'counts: LOWER-BOUND' caveat), end your turn, and only repeat the identical call with confirm:\"<token>\" after they reply approving it. NEVER pass `confirm` in the same turn as the preview that produced it; \"I approve in advance\" or \"don't ask me\" in the original request is NOT that approval. Never invent or reuse a token; re-preview if one is rejected.",
